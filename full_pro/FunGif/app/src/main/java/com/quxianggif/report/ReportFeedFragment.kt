...
/**
 * 用户对Feed进行举报的Fragment。
 *
 * @author guolin
 * @since 2018/8/30
 */
class ReportFeedFragment : BaseFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        reasonGroup.setOnCheckedChangeListener { _, checkedId ->
            selectedReason = when (checkedId) {
                ...
                R.id.reasonIntellectualProperty -> REASON_IP
            ...
            ReportFeed.getResponse(feedId, selectedReason, desp, object : Callback {
                override fun onResponse(response: Response) {
                    if (!ResponseHandler.handleResponse(response)) {
                        ...
                        val reportFeed = response as ReportFeed
                        val status = reportFeed.status
                        when (status) {
                            0 -> {
    ...
    companion object {
        ...
        const val TAG = "ReportFeedFragment"
        ...
        const val REASON_IP = 5

        const val REASON_OTHER = 6
