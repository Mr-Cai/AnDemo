...
class UserHomePageActivity : BaseActivity(), LoadDataListener {
    ...
    private fun setFollowersCountChange(changeCount: Int) {
        try {
            ...
            val followerCount =
                Integer.parseInt(followerInfo.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1])
            userFollowersText.text = String.format(
                GlobalUtil.getString(R.string.user_followers_count),
