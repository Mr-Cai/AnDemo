...
object Downloader {
     ...
     */
    open class DownloadTask internal constructor(private var l: DownloadListener) :
                    ...
                    if (responseBody != null) {
                        ...
                        while (bytes >= 0) {
                            if (!interrupt) {
                                ...
                                total += bytes
                                val downloadPercent =
                                    ((total + downloadedLength) * 1.0 / ((contentLength + downloadedLength) * 1.0) * 100).toInt()
        ...
        override fun onPostExecute(filePath: String?) {
            if (filePath != null) {
                ...
                if (e != null) {
                    ...
                    l.onFailure(
                        "Canceled by user or server response failed.",
                        RuntimeException("Canceled by user or server response failed.")
                    )
                }
            }
        }
