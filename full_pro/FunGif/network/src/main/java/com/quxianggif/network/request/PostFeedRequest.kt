...
class PostFeedRequest : Request() {
    ...
    override fun headers(builder: Headers.Builder): Headers.Builder {
        buildAuthHeaders(
            ...
            NetworkConst.GIF,
            NetworkConst.TOKEN,
            NetworkConst.IMG_WIDTH,
            NetworkConst.UID,
            NetworkConst.IMG_HEIGHT
        )
                ...
                if (!ResHandler.handleResponse(response)) {
                    ...
                    when (status) {
                        ...
                        10301 -> {
                            ...
                            logWarn(
                                "preparePost",
                                GlobalUtil.getResponseClue(status, preparePost.msg)
                            )
                            mCallback.onFailure(
                                PostFeedException(
                                    GlobalUtil.getResponseClue(
                                        status,
                                        preparePost.msg
                                    )
                                )
    ...
    private fun uploadGifCover(preparePost: PreparePost) {
        ...
        QiniuManager.upload(
            firstFramePath,
            key,
            preparePost.openUptoken,
            object : QiniuManager.UploadListener {
