...
class OpenSourceLoginActivity : LoginActivity() {
    ...
    override fun setupViews() {
        super.setupViews()
        val isStartWithTransition =
            intent.getBooleanExtra(LoginActivity.START_WITH_TRANSITION, false)
            ...
            override fun onResponse(response: Response) {
                if (!ResponseHandler.handleResponse(response)) {
                    ...
                    when (status) {
                        ...
                        10101 -> {
                            hideSoftKeyboard()
                            OpenSourceRegisterActivity.registerByPhone(
                                this@OpenSourceLoginActivity,
                                number,
                                code
                            )
