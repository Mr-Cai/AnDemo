...
class OpenSourceRegisterActivity : RegisterActivity(), TextView.OnEditorActionListener {
    ...
    override fun setupViews() {
        super.setupViews()
        if (intent.getStringExtra(INTENT_PHONE_NUMBER) == null || intent.getStringExtra(
                INTENT_VERIFY_CODE
            ) == null
        ) {
        ...
        if (!ResponseHandler.handleResponse(response)) {
            ...
            when (status) {
                0 -> {
                    logDebug(
                        TAG,
                        "token is " + register.token + " , getAvatar is " + register.avatar
                    )
                ...
                10105 -> {
                    ...
                    nicknameLayout.error =
                        GlobalUtil.getString(R.string.register_failed_nickname_is_used)
                }
                else -> {
                    logWarn(
                        TAG,
                        "Register failed. " + GlobalUtil.getResponseClue(status, register.msg)
                    )
