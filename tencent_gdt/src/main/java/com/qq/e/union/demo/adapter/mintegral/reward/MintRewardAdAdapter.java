package com.qq.e.union.demo.adapter.mintegral.reward;
...
import com.qq.e.union.demo.adapter.mintegral.util.MTGSDKInitUtil;
...
public class MintRewardAdAdapter extends BaseRewardAd {
    ...
    public MintRewardAdAdapter(Context context, String posID, String ext) {
        ...
        try {
            json = new JSONObject(ext);
            String appId = json.optString(KEY_APPID);
            String appKey = json.optString(KEY_APP);
            //step 1 : 初始化SDK
            MTGSDKInitUtil.initSDK(context, appId, appKey);
        } catch (JSONException e) {
            e.printStackTrace();
            Log.e(TAG, "MTGRewardVideoAdAdapter: do not get app id or key");
        }
    }
