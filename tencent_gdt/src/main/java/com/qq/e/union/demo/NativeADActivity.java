...
public class NativeADActivity extends Activity implements NativeAdListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        arrayAdapter = new PosIdArrayAdapter(this, android.R.layout.simple_spinner_item, getResources().getStringArray(R.array.gdt_native_ad_demo));
        arrayAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        $.id(R.id.id_spinner).adapter(arrayAdapter);
        $.id(R.id.id_spinner).itemSelected(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                arrayAdapter.setSelectedPos(position);
                $.id(R.id.nativePosId).text(getResources().getStringArray(R.array.gdt_native_ad_demo_value)[position]);
            }
    ...
    public void onADLoaded(List<NativeADDataRef> arg0) {
        if (arg0.size() > 0) {
            ...
            Log.d(TAG, String.format(Locale.getDefault(), "(pic_width,pic_height) = (%d , %d)", adItem
                    .getPictureWidth(), adItem.getPictureHeight()));
            Log.d(TAG, "eCPM = " + adItem.getECPM() + " , eCPMLevel = " + adItem.getECPMLevel());
        } else {
