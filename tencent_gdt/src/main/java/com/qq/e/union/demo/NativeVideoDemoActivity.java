...
public class NativeVideoDemoActivity extends Activity {
    ...
    private void initView() {
        ...
        mADInfoContainer = this.findViewById(R.id.ad_info_container);
    ...
    private void initNativeVideoAD() {
        NativeMediaADListener listener = new NativeMediaADListener() {
            ...
            public void onADLoaded(List<NativeMediaADData> adList) {
                if (adList.size() > 0) {
                    ...
                    Log.d(TAG, "eCPM = " + mAD.getECPM() + " , eCPMLevel = " + mAD.getECPMLevel());
                }
            }
        ...
        mADManager = new NativeMediaAD(getApplicationContext(), Constants.APPID, getPosId(), listener);
    ...
    private void initADUI() {
        ...
        if (patternType == AdPatternType.NATIVE_2IMAGE_2TEXT || patternType == AdPatternType.NATIVE_VIDEO) {
            ...
            mAQuery.id(R.id.native_3img_title).text(mAD.getTitle());
            mAQuery.id(R.id.native_3img_desc).text(mAD.getDesc());
        }
        updateADUI();
        /**
