...
class LikeStar @JvmOverloads constructor(context: Context, attrs: AttributeSet, attr: Int = 0) :
    ...
    private fun init(context: Context) {
        ...
        val interpolatorSet = ArrayList<Interpolator>() // 初始化插补器
        interpolatorSet.add(LinearInterpolator())
        interpolatorSet.add(AccelerateDecelerateInterpolator())
        interpolatorSet.add(AccelerateInterpolator())
        interpolatorSet.add(DecelerateInterpolator())
        val defaultIcon = ImageView(context)
        defaultIcon.setImageDrawable(starDrawable[0])
        defaultIcon.layoutParams = LayoutParams(WRAP_CONTENT, WRAP_CONTENT)
        defaultIcon.setOnClickListener {
            ...
            iconRandom.layoutParams = LayoutParams(WRAP_CONTENT, WRAP_CONTENT)
            addView(iconRandom)
            invalidate()
            val bezier = Bezier(
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        for (i in 0 until childCount) getChildAt(i).layout(
            (widthM - getChildAt(i)
    ...
    inner class Bezier(private val pf1: PointF, private val pf2: PointF) : TypeEvaluator<PointF> {
        override fun evaluate(f: Float, start: PointF, endValue: PointF): PointF {
            val pointCur = PointF()
            pointCur.x = startP.x * (1.0 - f).pow(3.0).toFloat() +
                    (3F * pf1.x * f * (1 - f) * (1 - f)) +
                    (3F * pf2.x * (1 - f) * f * f) +
                    endValue.x * f * f * f // 实时计算最新的点X坐标
            pointCur.y = startP.y * (1.0 - f).pow(3.0).toFloat() +
                    (3F * pf1.y * f * (1 - f) * (1 - f)) +
                    (3F * pf2.y * (1 - f) * f * f) +
